...
/**
 * Arionbanki IOBWS Proxy — Requirement 17
 *
 ...
 *   ARIONBANKI_API_KEY            (optional — overrides shared key)
 *   IOBWS_API_KEY                 (required if no bank-specific key)
 *   ARIONBANKI_CLIENT_CERT_PFX    (optional — overrides shared cert)
 *   CLIENT_CERT_PFX               (required if no bank-specific cert)
 *   ARIONBANKI_CLIENT_CERT_PASSWORD (optional)
...
function resolveApiKey() {
  return process.env.ARIONBANKI_API_KEY || process.env.IOBWS_API_KEY || '';
}

function loadCert() {
  if (_certCache) return _certCache;
  const pfxB64  = process.env.ARIONBANKI_CLIENT_CERT_PFX       || process.env.CLIENT_CERT_PFX      || '';
  const pfxPass = process.env.ARIONBANKI_CLIENT_CERT_PASSWORD   || process.env.CLIENT_CERT_PASSWORD || '';
...
module.exports = async function (context, req) {
  ...
  if (!host || !ALLOWLIST.has(host)) {
    context.res = { status: 400, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'serviceUrl host is not in the Arionbanki allowlist' }) };
    return;
  }
