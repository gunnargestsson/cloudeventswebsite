...
### Requirement 23: Open Mirror Metadata Format Fix
**Folder**: `requirement-23-open-mirror-metadata-fix/`
**Status**: 🔧 Bug Fix — Ready for Implementation

Fix the Fabric Open Mirroring metadata file path and JSON format generated by `api/mirror/index.js`.

**Problems:**
1. **Wrong path** — file is uploaded to `Tables/{tableName}/_metadata/DDL.json`; correct path is `Tables/{tableName}/_metadata.json` (no subfolder)
2. **Wrong format** — file uses a SQL DDL schema structure; correct format is the Fabric Open Mirroring `SchemaDefinition` format

**Changes required in `api/mirror/index.js`:**
- Replace `mapDdlType()` → `mapFabricType()` returning Fabric type strings (`String`, `Double`, `Int64`, `Int32`, `Boolean`, `IDate`, `ITime`, `DateTime`)
- Replace `buildDdl()` to emit `{ keyColumns, fileDetectionStrategy, SchemaDefinition: { Columns }, fileFormat }` structure
- Column names in `SchemaDefinition.Columns` use plain washed names (no `-{fieldNo}` suffix)
- System fields use correct casing: `timestamp` (lowercase), `systemId` (lowercase), `SystemCreatedAt/By/ModifiedAt/By` (no number suffix)
- `keyColumns`: `["systemId", "$Company"]` for per-company tables, `["systemId"]` for global
- `uploadDdl()`: change path from `pathJoin("Tables", safeTableName, "_metadata", "DDL.json")` to `pathJoin("Tables", safeTableName, "_metadata.json")`

**Files**:
- `SPECIFICATION.md` — Complete before/after format comparison, new type mapping table, updated `buildDdl()` and `uploadDdl()` implementations, testing checklist

---

### Requirement 24: Open Mirror — Deleted Records Support
**Folder**: `requirement-24-open-mirror-deleted-records/`
**Status**: 📝 Specification — Ready for Implementation

Extend `runMirror()` in `api/mirror/index.js` so that each run also exports deleted records for the same adjusted timeframe and uploads them as a separate `_deleted.csv` file alongside the regular CSV.

**New flow:**
1. Count modified records (`Data.Records.Get`) — if > 0, call `CSV.Records.Get` to get `bcTime`
2. Determine `confirmedIso` = `bcTime || endIso`
3. Count deleted records (`Deleted.Records.Get`, window `[previousTs..confirmedIso]`) — if > 0, call `CSV.DeletedRecords.Get`
4. Skip only if both counts are 0; a run with only deletions still writes a timestamp
5. Write integration timestamp, then upload both files; reverse on error

**Files written per run:**
- `Tables/{tableName}/{yyyy}/{MM}/{dd}/{stamp}.csv` — modified records (if any)
- `Tables/{tableName}/{yyyy}/{MM}/{dd}/{stamp}_deleted.csv` — deleted records (if any)

**New helper functions**: `getDeletedRecordCount()`, `getCsvDeletedRecords()`

**Files**:
- `SPECIFICATION.md` — Full flow description, API reference for `Deleted.Records.Get` and `CSV.DeletedRecords.Get` (including the `fromDate`/`toDate` parameter difference), complete replacement `runMirror()` implementation, return value changes, testing checklist
